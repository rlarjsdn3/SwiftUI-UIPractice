...
struct Calculator {
    ...
    private var containsDecimal: Bool {
        return getNumberString(forNumber: newNumber).contains(".")
    }
    ...
    mutating func setDigit(_ digit: Digit) {
        if containsDecimal && digit == .zero {
            ...
            let numberString: String = getNumberString(forNumber: newNumber)
    ...
    private func getNumberString(forNumber number: Decimal?, withCommas: Bool = false) -> String {
        var numberString: String = (withCommas ? number?.formatted(.number) : number.map(String.init)) ?? "0"
